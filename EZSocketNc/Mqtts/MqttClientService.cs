...
using System.Net;
using EquipmentDriver.Mqtts.Dtos;
using EquipmentDriver.Mqtts;
using EquipmentDriver.Equipment.Configs;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.Logging.Abstractions;

namespace EquipmentDriver.Mqtts
{
    public class MqttClientService : Volo.Abp.DependencyInjection.ISingletonDependency, IMqttClientService
    {
        public IConfiguration _config { get; set; }
        public ILogger<MqttClientService> _logger { get; set; }
        ...
        public MqttClientService(IConfiguration config, ILogger<MqttClientService> logger = null)
        {
            _config = config;
            _logger = logger;// NullLogger<MqttClientService>.Instance;//
        ...
        public async Task InitMachineList()
        {
            ...
            if (!int.TryParse(_config["CncService: MqttHeartBeat"], out _heartBeat))
            {
                _heartBeat = 50;
            }
        }
        ...
        public async Task StartAsync()
        {
            ...
            try
            {
                ...
                var options = new MqttClientOptionsBuilder()
                    ...
                    // QoS 0：消息最多传送一次。如果当前客户端不可用，它将丢失这条消息。 AtMostOnce
                    // QoS 1：消息至少传送一次。 AtLeastOnce
                    // QoS 2：消息只传送一次。ExactlyOnce
                    .WithWillQualityOfServiceLevel(Qos)
                    .WithCredentials(mqttConfig.User, mqttConfig.Password).Build();
                _logger.LogError($"[mqtt]Create Mqtt client:{amout++}");
            ...
            {
                _logger.LogError($"[mqtt]Start Mqtt Error:{ex.Message}");
            }
        }
        ...
        private async Task _client_DisconnectedAsync(MqttClientDisconnectedEventArgs arg)
        {
            await Task.Run(async () =>
            {
                try
                {
                    _logger.LogInformation("[mqtt]MQTT client disconnect");
                    Thread.Sleep(3000);
                    await RestartAsync();
                }
                catch (Exception ex)
                {
                    _logger.LogError($"[mqtt]reconnect mqtt:{ex.Message}");
                }
            });
        }
        ...
        {
            return Task.Run(() => { _logger.LogInformation("[mqtt]MQTT client connect"); });
        }
        ...
        public async Task StopAsync()
        {
            ...
            {
                _logger.LogError($"[mqtt]MQTT client stop：{ex.Message}\r\n{ex.StackTrace}");
            }
        }
        ...
        private async void StartHeartBeat()
        {
            ...
            while (!cts.IsCancellationRequested)
            {
                if (IsConnected)
                {
                    ...
                    {
                        if (!CheckOtherSysConnect) //需要根据第三方系统的连接性来判断是否需要发送心跳包
                        {
                            if (!cur.IsConnected)
                            {
                        ...
                        try
                        {
                            ...
                            var message = new MqttApplicationMessageBuilder()
                                  .WithTopic(cur.Topic)
                                  .WithPayload(Newtonsoft.Json.JsonConvert.SerializeObject(msg, JsonSerializerSettings))
                                  .WithQualityOfServiceLevel(Qos)
                                  .WithRetainFlag().Build();
                            var result = await _client.PublishAsync(message, cts);
                            if (!result.IsSuccess)
                            {
                                _logger.LogError($"[mqtt]发送心跳包失败,ReasonCode:{result.ReasonCode}\r\n{result.ReasonString}");
                            }
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError($"[mqtt]发送心跳包异常:{ex.Message}\r\n{ex.StackTrace}");
                        }
                    }
        ...
        public void SetBaseInfo(BaseMsg msg, IEquipmentConfig equipment)
        {
            ...
            if (string.IsNullOrWhiteSpace(msg.HostName))
                msg.HostName = equipment.HostName;
            if (string.IsNullOrWhiteSpace(msg.HostName))
        ...
        public async ValueTask<bool> SendAsync(BaseMsg msg, IEquipmentConfig equipment, int tryTimes = 3)
        {
            ...
            {
                _logger.LogError($"[mqtt][{equipment.Topic}][{tryTimes}]send Error :{ex.Message}\r\n{ex.StackTrace}");
                //失败重试3次（延迟N次方秒）
                var time = (equipment.Retries - tryTimes + 1);
                await Task.Delay(1000 * time);
                return await SendAsync(msg, equipment, --tryTimes);

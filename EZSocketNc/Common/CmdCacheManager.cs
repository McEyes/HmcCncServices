...
namespace EZSocketNc.Commons
{
    public class CmdCacheManager
    {
        ...
        /// <returns></returns>
        public bool IsExistsSameRecipe(BaseMsg Msg)
        {
            lock (_lock)
            {
                if (cmdRecordInfos.Count == 0) return false;
                return cmdRecordInfos.Exists(p => p.MachineId == Msg.HostName && p.LastTime > DateTime.Now.AddMinutes(-CmdRepeatCheckTime) && p.RecipeName == Msg.ToString());
            }
        }
        ...
        private async void FlushData()
        {
            ...
            //if (!int.TryParse(_config["PlcService:CmdRepeatCheckTime"], out CmdRepeatCheckTime)) CmdRepeatCheckTime = 1;
            //}
            //catch { }
            //});
        }
